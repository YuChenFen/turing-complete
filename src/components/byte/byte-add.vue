<template>
    <div>
        <div class="bit-add" :style="`transform: rotate(${rotate}deg);`" @dblclick="dblclick">
            ...
            <p style="position: absolute;top: 50%;right: 10%;transform: translate(0%, -50%);font-size: 0.5em;">
                8</p>
        </div>
...
function update() {
    ...
    if (input_b.value != void 0) {
        flag = true;
        sum += input_b.value & 255;
    }
    if (input_c.value != void 0) {
        flag = true;
        sum += input_c.value & 255;
    }
    if (flag) {
        updateNodeData(props.id, { output_sum: sum & 255 });
        updateNodeData(props.id, { output_carry: (sum >> 8) & 1 });
    } else {
