<template>
    <div>
        <div class="byte-or" :style="`transform: rotate(${rotate}deg);`" @dblclick="dblclick">
            <svg-icon name="byte-or" style="width: 30px;height: 30px;"></svg-icon>
            <p
                style="position: absolute;top: 50%;left: 50%;transform: translate(-50%, -60%) scale(0.5);font-size: 0.5em;">
                OR8</p>
        </div>
...
watch(input_a, (input_a) => {
    ...
    if (input_b.value != undefined) {
        updateNodeData(props.id, { output_a: (input_a | input_b.value) & 255 });
    }
})
watch(input_b, (input_b) => {
    ...
    if (input_a.value != undefined) {
        updateNodeData(props.id, { output_a: (input_a.value | input_b) & 255 });
    }
})
