<template>
    <div>
        <div class="byte-not" :style="`transform: rotate(${rotate}deg);`" @dblclick="dblclick">
            <svg-icon name="byte-not" style="width: 30px;height: 30px;"></svg-icon>
            <p
...
watch(input_a, (input_a) => {
    ...
    updateNodeData(props.id, { output_a: 255 - (input_a & 255) });
})
