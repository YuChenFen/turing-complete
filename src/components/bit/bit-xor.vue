<template>
    <div>
        <div class="bit-xor" :style="`transform: rotate(${rotate}deg);`" @dblclick="dblclick">
            <svg-icon name="bit-xor" style="width: 30px;height: 30px;"></svg-icon>
            <p
                style="position: absolute;top: 50%;left: 50%;transform: translate(-50%, -70%) scale(0.5);font-size: 0.5em;">
                XOR</p>
        </div>
...
watch(input_a, (input_a) => {
    ...
    if (input_b.value != undefined) {
        updateNodeData(props.id, { output_a: input_a ^ input_b.value });
    }
})
watch(input_b, (input_b) => {
    ...
    if (input_a.value != undefined) {
        updateNodeData(props.id, { output_a: input_a.value ^ input_b });
    }
})
