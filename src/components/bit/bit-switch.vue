<template>
    <div>
        <div class="bit-switch" :style="`transform: rotate(${rotate}deg);`" @dblclick="dblclick">
            <svg-icon v-show="!input_switch" name="bit-switch-off" style="width: 30px;height: 30px;"></svg-icon>
            <svg-icon v-show="input_switch" name="bit-switch-on" style="width: 30px;height: 30px;"></svg-icon>
        </div>
...
watch(input_switch, (input_switch) => {
    ...
    if (input_switch & 1) {
        updateNodeData(props.id, { output_a: input_a.value & 1 });
    } else {
        updateNodeData(props.id, { output_a: void 0 });
    }
})
