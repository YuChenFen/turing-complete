<template>
    <div>
        <div class="bit-or" :style="`transform: rotate(${rotate}deg);`" @dblclick="dblclick">
            <svg-icon name="bit-or" style="width: 30px;height: 30px;"></svg-icon>
            <p
...
watch(input_a, (input_a) => {
    ...
    if (input_b.value != undefined) {
        updateNodeData(props.id, { output_a: (input_a | input_b.value) & 1 });
    }
})
watch(input_b, (input_b) => {
    ...
    if (input_a.value != undefined) {
        updateNodeData(props.id, { output_a: (input_a.value | input_b) & 1 });
    }
})
